---
title: "Differential expression analysis for platelet gene in different cohort"
author: "Romane Cathelin"
output: html_document
---


```{r libraries}
packages.list <- c("dplyr", "tibble", "ggplot2", "DESeq2", "kableExtra", "viridis")
for (package in packages.list) {
  if (!require(package, character.only = TRUE)) {
    install.packages(package)
  }
}
sapply(packages.list, library, character.only = TRUE)
source("/gpfs/home/cather01/platelet/matt_deseq_function.R")

```

```{r paths}
input.path <- "/gpfs/data/abl/home/cather01/rugglesLab/PACE/DATA/"
output.path <- "/gpfs/data/abl/home/cather01/rugglesLab/PACE/"
```

```{r young_vs_old}
#### Young vs Old ###
data = read.csv(paste0(input.path, "countmatrix_plt_PACE.csv"), row.names = 1)
metadata = read.csv(paste0(input.path, "pace_and_thr_metadata.csv"), row.names = 1)
metadata <- metadata[rownames(metadata) %in% colnames(data), ]

# check the distribution 
p <- ggplot(metadata, aes(x=age)) + 
  geom_histogram( fill="#69b3a2", color="#e9ecef", alpha=0.9)+theme_bw()+ scale_x_continuous(breaks = seq(0, 100, 5))

# Delete outliers 
metadata <- as.data.frame(metadata[rownames(metadata) != "PACE011", ])
data <- data[, colnames(data) != "PACE011"]

#Transform age in young and old
metadata <- metadata %>%
  mutate(age_group = ifelse(age < 73, "young", "old"))

#Transform column into factor
met <- as.data.frame(metadata$age_group)
rownames(met)<-rownames(metadata)
colnames(met) <- c("age")
met$age <- as.factor(met$age)


young_data <- met[met$age == "young", ]
old_data <- met[met$age == "old", ]
length(young_data)
length(old_data)


#Verification
all(colnames(data) %in% rownames(met))
all(colnames(data) == rownames(met))

#Construct DESEQ2 object
dds <- DESeqDataSetFromMatrix(countData = data, 
                               colData = met,
                               design = ~ age)
dds


#pre filtering --> removing rows with low gene counts
# keeping rows that have at least 10 reads total
dds2 <- rna_preprocessing(dds, "/gpfs/data/abl/home/cather01/rugglesLab/PACE/plt_deseq_preprocess")


#Set the factor level (3 level = 3 categories d'ages)
dds2$age <- relevel(dds$age, ref = "young")


#Run DESeq
dds2 <- DESeq(dds2)
res2 <- results(dds2)
summary(res2)
# saveRDS(dds2, "/gpfs/data/abl/home/cather01/rugglesLab/plt_vs_wb/dds_plt.rds")

#Check how many gene are significant
sum(res2$padj < 0.2, na.rm=TRUE)
length(which(res2$pvalue < 0.05))

```




```{r}

resultsNames(dds2)
res_tableOE <- lfcShrink(dds2, coef="age_old_vs_young")

EnhancedVolcano(toptable = res2,
                x = "log2FoldChange",
                y = "padj",
                lab = rownames(res2),
                # xlim = c(-5, +5),
                # ylim = c(0, 10),
                pCutoff = 0.2,
                FCcutoff = 0.5,
                # pointSize = 2.0,
                title = "Young patient (<68) vs old \n Fold change cutoff = 0.5, p-adj cutoff = 0.2",
                legendLabels=c(
                  'Not significant',
                  'Pass Log2 fold-change cutoff',
                  'Pass p-adj cutoff',
                  'Pass both p-adj & Log2 fold change')
                )
## MA plot 
DESeq2::plotMA(res2)

colData(dds2)


res.df <- as.data.frame(res2)


res.df <- res.df[(res.df$baseMean > 150) & (abs(res.df$log2FoldChange) > 1.3),]
res.df <- res.df[(res.df$padj < 0.2) & (abs(res.df$log2FoldChange) > 0.5),]
res.df <- res.df[(abs(res.df$log2FoldChange) > 0.5),]
res.df <- res.df[(!is.na(res.df$padj) & res.df$padj < 0.1), ]


# Extract normalized counts for significant genes
mat <- counts(dds2, normalized = TRUE)[rownames(res.df),]
mat.z <- t(scale(t(mat)))

# Annotate columns (patients) with sex information for the Heatmap function
Heatmap(mat.z, 
        cluster_rows = TRUE, 
        cluster_columns = TRUE, 
        column_labels = colnames(mat.z), 
        name = "Z-score",
        column_title = "Patient",
        column_dend_reorder = TRUE,
        column_dend_side = "top",
        column_title_gp = gpar(fontsize = 9, fontface = "bold"),
        heatmap_legend_param = list(title = "Z-score"),
        top_annotation = HeatmapAnnotation(Sex = anno_text(dds$sex, which = "col", rot = 90))
)

# Generate heatmap using pheatmap with additional customization
pheatmap(mat.z,
         cluster_rows = TRUE,
         cluster_cols = TRUE,
         labels_col = colnames(mat.z),
         annotation_col = data.frame(Age = dds2$age),
         annotation_colors = list(Age = c("young" = "#3498db", "old" = "#e74c3c")),
         main = "Gene Expression Heatmap",
         border_color = "white",
         show_colnames = TRUE,
         show_rownames = TRUE
)


# Log transformation
mat_log <- log2(mat + 1)  # Adding 1 to avoid log(0)

# Create a pheatmap using log-transformed values
pheatmap(mat_log,
         cluster_rows = TRUE,
         cluster_cols = TRUE,
         labels_col = colnames(mat_log),
         annotation_col = data.frame(Age = dds2$age),
         annotation_colors = list(Age = c("young" = "blue", "old" = "pink")),
         main = "Heatmap",
         fontsize_row = 8,
         fontsize_col = 8
)





```




```{r male_vs_female}
#### Male vs Female###
data = read.csv(paste0(input.path, "countmatrix_plt_PACE.csv"), row.names = 1)
metadata = read.csv(paste0(input.path, "pace_and_thr_metadata.csv"), row.names = 1)
metadata <- metadata[rownames(metadata) %in% colnames(data), ]

p <- ggplot(metadata, aes(x=sex)) + 
  geom_histogram( fill="#69b3a2", color="#e9ecef", alpha=0.9)+theme_bw()+ scale_x_continuous(breaks = seq(0, 100, 5))
names(metadata)
metadata$patient <- rownames(metadata)

# Delete outliers 
metadata <- as.data.frame(metadata[rownames(metadata) != "PACE011", ])
data <- data[, colnames(data) != "PACE011"]

out.path <- "/gpfs/data/abl/home/cather01/rugglesLab/"
tabular.stat.table <- metadata %>%
  dplyr::select("age", "sex", "ethnicity", "smoking", "bmi", "diabetes","CAD","hypertension", "hyperlipidemia") %>%
  kbl("html") %>%
  kable_styling(bootstrap_options = "striped", full_width = F) %>%
  kable_minimal() %>%
  save_kable(paste0(out.path, "stat_table.html"))

#Transform column into factor
met <- as.data.frame(metadata$sex)
rownames(met)<-rownames(metadata)
colnames(met) <- c("sex")
met$sex <- as.factor(met$sex)


female_data <- met[met$sex == "Female", ]
male_data <- met[met$sex == "Male", ]
length(female_data)
length(male_data)


#Verification
all(colnames(data) %in% rownames(met))
all(colnames(data) == rownames(met))

#Construct DESEQ2 object
dds <- DESeqDataSetFromMatrix(countData = data, 
                               colData = met,
                               design = ~ sex)
dds


 # Filter results
# Filtering 
dds <- rna_preprocessing(dds, "./")

# Run DESeq
dds <- DESeq(dds)
res <- results(dds)
summary(res)

sum(res$padj < 0.2, na.rm=TRUE)

EnhancedVolcano(toptable = res,
                x = "log2FoldChange",
                y = "padj",
                lab = rownames(res),
                 xlim = c(-4, +8),
                 ylim = c(0, 10),
                pCutoff = 0.2,
                FCcutoff = 0.5,
                # pointSize = 2.0,
                title = "Male vs female patient \n Fold change cutoff = 0.5, p-adj cutoff = 0.2",
                legendLabels=c(
                  'Not significant',
                  'Pass Log2 fold-change cutoff',
                  'Pass p-adj cutoff',
                  'Pass both p-adj & Log2 fold change')
                )
res.df <- as.data.frame(res)
na.omit(res.df)
res.df <- res.df[(res.df$padj < 0.02) & (abs(res.df$log2FoldChange) > 0.5),]
res.df <- res.df[(abs(res.df$log2FoldChange) > 2),]
res.df <- res.df[(!is.na(res.df$padj) & res.df$padj < 0.05), ]

# Extract normalized counts for significant genes
mat <- counts(dds, normalized = TRUE)[rownames(res.df),]
mat.z <- t(scale(t(mat)))

# Annotate columns (patients) with sex information for the Heatmap function
Heatmap(mat.z, 
        cluster_rows = TRUE, 
        cluster_columns = TRUE, 
        column_labels = colnames(mat.z), 
        name = "Z-score",
        column_title = "Patient",
        column_dend_reorder = TRUE,
        column_dend_side = "top",
        column_title_gp = gpar(fontsize = 9, fontface = "bold"),
        heatmap_legend_param = list(title = "Z-score"),
        top_annotation = HeatmapAnnotation(Sex = anno_text(dds$sex, which = "col", rot = 90))
)

# Generate heatmap using pheatmap with additional customization
pheatmap(mat.z,
         cluster_rows = TRUE,
         cluster_cols = TRUE,
         labels_col = colnames(mat.z),
         annotation_col = data.frame(Sex = dds$sex),
         annotation_colors = list(Sex = c("Male" = "#3498db", "Female" = "#e74c3c")),
         main = "Gene Expression Heatmap",
         border_color = "white",
         show_colnames = TRUE,
         show_rownames = TRUE
)


# Log transformation
mat_log <- log2(mat + 1)  # Adding 1 to avoid log(0)

# Create a pheatmap using log-transformed values
pheatmap(mat_log,
         cluster_rows = TRUE,
         cluster_cols = TRUE,
         labels_col = colnames(mat_log),
         annotation_col = data.frame(Sex = dds$sex),
         annotation_colors = list(Sex = c("Male" = "blue", "Female" = "pink")),
         main = "Heatmap",
         fontsize_row = 8,
         fontsize_col = 8
)



## Filter out outliers
filtered_res.df <- res.df[!(abs(res.df$log2FoldChange) > 10 | res.df$pvalue > 1e-10), ]

# Volcano plot with outliers removed
volcano_plot <- ggplot(data = filtered_res.df, aes(x = log2FoldChange, y = -log10(pvalue))) +
  geom_point(size = 2, color = ifelse(abs(filtered_res.df$log2FoldChange) > 2 & filtered_res.df$pvalue < 0.05, "red", "black")) +
  labs(title = "Volcano Plot (Outliers Removed)", x = "Log2 Fold Change", y = "-log10(p-value)")

print(volcano_plot)


# Example MA plot
ma_plot <- ggplot(res.df, aes(x = log2(baseMean), y = log2FoldChange)) +
  geom_point(size = 2, color = ifelse(res.df$pvalue < 0.05, "red", "black")) +
  labs(title = "MA Plot", x = "log2(Average Expression)", y = "log2(Fold Change)")

print(ma_plot)


```
```{r else}


# Filtering 
dds2 <- rna_preprocessing(dds, "/gpfs/data/abl/home/cather01/rugglesLab/PACE/plt_deseq_preprocess")
dds2
# Plot histogram with ggplot2
#Run DESeq
dds2 <- DESeq(dds2)
res2 <- results(dds2)
summary(res2)

max(res2$lfcSE)
res2 <- res2[res2$log2FoldChange < 9.947548,]
# saveRDS(dds2, "/gpfs/data/abl/home/cather01/rugglesLab/plt_vs_wb/dds_plt.rds")

#Check how many gene are significant
sum(res2$padj < 0.1, na.rm=TRUE)
length(which(res2$pvalue < 0.01))


## MA plot 
DESeq2::plotMA(res2)



resultsNames(dds2)
res_tableOE <- lfcShrink(dds2, coef="sex_Male_vs_Female")


DESeq2::plotMA(res2)
DESeq2::plotMA(res_tableOE)

res_tableOE_tb <- res_tableOE %>%
  data.frame() %>%
  rownames_to_column(var="gene") %>% 
  as_tibble()

sigOE <- res_tableOE_tb %>%
        filter(padj < 0.2 & abs(log2FoldChange) > 0.45)

list.gene <- get_fc_list(res2)
df.enri <- rna_enrichment(list.gene,  "/gpfs/data/abl/home/cather01/rugglesLab/PACE/enrich")
df.gsea <- gsea_analysis(list.gene,  "/gpfs/data/abl/home/cather01/rugglesLab/PACE/gsea_analysis")
```

